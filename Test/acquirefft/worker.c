...
int rp_spectr_worker_init(int binning_factor)
{
    int ret_val;
    ...
    if(rp_create_signals(&rp_spectr_signals, binning) < 0)
        return -1;

    rp_cleanup_signals(&rp_tmp_signals);

    if(rp_create_signals(&rp_tmp_signals, binning) < 0) {
        rp_cleanup_signals(&rp_spectr_signals);
        return -1;
    }
...
//  fprintf(stderr, "worker.c rp_spectr_worker_init SPECTR_FPGA_SIG_LEN=%d c_dsp_sig_len=%d\n", SPECTR_FPGA_SIG_LEN, c_dsp_sig_len);
...
int rp_spectr_get_signals(float ***signals, rp_spectr_worker_res_t *result, int binning)
{
    float **s = *signals;
    pthread_mutex_lock(&rp_spectr_sig_mutex);
    ...
    memcpy(&s[0][0], &rp_spectr_signals[0][0], sizeof(float)*(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */);
    memcpy(&s[1][0], &rp_spectr_signals[1][0], sizeof(float)*(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */);
    memcpy(&s[2][0], &rp_spectr_signals[2][0], sizeof(float)*(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */);
...
int rp_spectr_set_signals(float **source, rp_spectr_worker_res_t result, int binning)
{
    pthread_mutex_lock(&rp_spectr_sig_mutex);
    memcpy(&rp_spectr_signals[0][0], &source[0][0], sizeof(float)*(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */);
    memcpy(&rp_spectr_signals[1][0], &source[1][0], sizeof(float)*(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */);
    memcpy(&rp_spectr_signals[2][0], &source[2][0], sizeof(float)*(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */);
...
***/
        ...
        //fprintf(stderr, "CALL rp_spectr_fft\n");
        rp_spectr_fft(&rp_cha_in[0], &rp_chb_in[0], //input 16k
                      (double **)&rp_cha_fft, (double **)&rp_chb_fft);//output 8k
        
        //fprintf(stderr, "CALL rp_spectr_decimate\n");
        rp_spectr_decimate(&rp_cha_fft[0], &rp_chb_fft[0], 
                           (float **)&rp_tmp_signals[1], 
                           (float **)&rp_tmp_signals[2],
                           binning);
                           /***
                           c_dsp_sig_len,//inlen, 8k
                           SPECTR_OUT_SIG_LEN);//outlen, 2k
                           ***/
        
        //fprintf(stderr, "CALL rp_spectr_cnv_to_dBm\n");
...
***/
        rp_spectr_set_signals(rp_tmp_signals, tmp_result, binning);
