...
int rp_spectr_prepare_freq_vector(float **freq_out, double f_s, float freq_range, int binning)
{
    int i,j,step;
    float *f = *freq_out;
    ...
    step = binning;//WAS: (int)round((float)c_dsp_sig_len / (float)SPECTR_OUT_SIG_LEN);
    if(step < 1)
        step = 1;

    for(i = 0, j = 0; i < c_dsp_sig_len/binning/*was SPECTR_OUT_SIG_LEN*/; i++, j+=step) {
        /* We use full FPGA signal length range for this calculation, even though
...
//                     int in_len, int out_len)
{
    int step;
    int i, j;
    int out_len = SPECTR_FPGA_SIG_LEN / 2 / binning;
    float *cha_o = *cha_out;
    float *chb_o = *chb_out;

  //fprintf(stderr, "dsp.c rp_spectr_decimate out_len=%d\n", out_len);

    if(!cha_in || !chb_in || !*cha_out || !*chb_out)
        return -1;

    step = binning;//(int)round((float)in_len / (float)out_len);
    //fprintf(stderr, "dsp.c rp_spectr_decimate step=%d\n", step);//ERG
    if(step < 1)
        step = 1;

    for(i = 0, j = 0; i < out_len; i++, j+=step) {
        int k=j;
        //fprintf(stderr, "dsp.c rp_spectr_decimate ## I=%d\n", i);//ERG
	...
	/* Conversion factor from ADC counts to Volts */
        ...
        //fprintf(stderr, "dsp.c rp_spectr_decimate k=%d\n", k);//ERG
                         ...
                         int binning)
{
    ...
    for(i = 0; i < (SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */; i++) {
	...
	if ((ixxa>=0) && (ixxa<(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */)) 
	 {
	   cha_pwr+=pow(10.0,cha_o[ixxa]/10.0);
	 }
	if ((ixxb>=0) && (ixxb<(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */)) 
	 {
    ...
    *peak_freq_cha = ((float)max_pw_idx_cha / (float)(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */ * 
                      freq_smpl  / 2) / unit_div;
    *peak_power_chb = max_pw_chb;
    *peak_freq_chb = ((float)max_pw_idx_chb / (float)(SPECTR_FFT_SIG_LEN/binning) /* was SPECTR_OUT_SIG_LEN */ * 
                      freq_smpl / 2) / unit_div;
